...
import { Foreigntraining } from '../../../models/Foreigntraining';


@Component({
  selector: 'app-foreigntraining',
  templateUrl: './foreigntraining.component.html',
  styleUrls: ['./foreigntraining.component.scss']
})
export class ForeigntrainingComponent implements OnInit {
  foreigntrainingForm:FormGroup;
  ...
  ngOnInit() {
    this.foreigntrainingForm = this.formBuilder.group({
  ...
  async saveDocuments(documentForm:FormGroup) {
    this.foreigntraining = new Foreigntraining();
    if(documentForm.valid) {
      this.foreigntraining = documentForm.value;
      console.log(this.foreigntraining);
    }
