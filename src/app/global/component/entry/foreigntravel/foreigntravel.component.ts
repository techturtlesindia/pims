...
@Component({
  selector: 'app-foreigntravel',
  templateUrl: './foreigntravel.component.html',
  styleUrls: ['./foreigntravel.component.scss']
})
export class ForeigntravelComponent implements OnInit {
  foreigntravelForm:FormGroup;
  ...
  ngOnInit() {
    this.foreigntravelForm = this.formBuilder.group({
  ...
  async saveDocuments(documentForm:FormGroup) {
    this.foreigntravel = new Foreigntravel();
    if(documentForm.valid) {
      this.foreigntravel = documentForm.value;
      console.log(this.foreigntravel);
    }
